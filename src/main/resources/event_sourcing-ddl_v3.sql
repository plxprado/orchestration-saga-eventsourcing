...
-- DROP SEQUENCE eventsourcing.sq_command_business_context_id;
...
ALTER SEQUENCE eventsourcing.sq_command_business_context_id OWNER TO postgres;
GRANT ALL ON SEQUENCE eventsourcing.sq_command_business_context_id TO postgres;
...
-- eventsourcing.command_business_context definition
...
CREATE TABLE eventsourcing.saga_roudmap (
	...
	CONSTRAINT saga_roudmap_unique UNIQUE (command_id),
...
-- eventsourcing.saga_roudmap_item definition

-- Drop table

-- DROP TABLE eventsourcing.saga_roudmap_item;

CREATE TABLE eventsourcing.saga_roudmap_item (
	id int8 NOT NULL,
	...
	CONSTRAINT saga_roudmap_item_saga_roudmap_fk FOREIGN KEY (saga_roudmap_id) REFERENCES eventsourcing.saga_roudmap(id)
);

-- Permissions

ALTER TABLE eventsourcing.saga_roudmap_item OWNER TO postgres;
GRANT ALL ON TABLE eventsourcing.saga_roudmap_item TO postgres;
...
CREATE TABLE eventsourcing.saga_event_stream (
	...
	CONSTRAINT saga_event_aggregation_unique UNIQUE (status, saga_item_id, aggregation_id),
	...
	CONSTRAINT saga_event_stream_saga_roudmap_item_fk FOREIGN KEY (saga_item_id) REFERENCES eventsourcing.saga_roudmap_item(id)
);
